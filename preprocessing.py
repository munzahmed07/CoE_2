...
def extract_scene(scene, scene_idx):
    ...
    for f_idx in range(start_idx, start_idx + TOTAL_FRAMES):
        f_agents = agents[frames[f_idx]["agent_index_interval"][0] : frames[f_idx]["agent_index_interval"][1]]
        match = [a for a in f_agents if a["track_id"] == track_id]
        if match:
            trajectory.append(match[0]["centroid"])
        else:
            logging.warning(f"Scene {scene_idx}: missing frame {f_idx} for agent.")
            return None
